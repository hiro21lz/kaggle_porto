...
if __name__ == '__main__':
    ...
    x_train = df.drop('target', axis=1) #avoid overfitting
    y_train = df['target'].values #pandas->numpy


    use_cols = x_train.columns.values

    logger.info('train columns: {}'.format(use_cols.shape))
...
#        clf = pickle.load(f)
 ...
 #       pickle.dump(pred_test, f, -1)
    ...
    df_submit.to_csv(DIR + 'submit_cv.csv', index=False)
